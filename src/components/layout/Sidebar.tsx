...
import { ChevronDown, ChevronsLeft, Building, Building2, CreditCard, Database, ShieldAlert, UserPlus } from "lucide-react";
      ...
      >
        ...
        <nav className="scrollbar-none flex-1 overflow-y-auto px-2.5 py-3">
          <ul className="flex flex-col gap-0.5">
            ...
            {isSuperAdmin && (
              <li className="group relative">
                <Link
                  ...
                  className={cn(
                    "flex items-center gap-3 rounded-[var(--radius-md)] px-3 py-2.5 text-sm font-medium transition-colors",
                    pathname.startsWith("/super-admin/onboarding")
                      ? "bg-[var(--color-primary)]/10 text-[var(--color-primary)]"
                      : "text-[var(--color-text-secondary)] hover:bg-[var(--color-bg)] hover:text-[var(--color-text)]",
                    collapsed && "lg:justify-center"
                  )}
                >
                  ...
                  {!collapsed && <span className="truncate">Society Onboarding</span>}
                </Link>
              </li>
            )}
            {isSuperAdmin && (
            ...
            {isSuperAdmin && (
              <li className="group relative">
                <Link href="/super-admin/security" onClick={onCloseMobile} className={cn(
                  "flex items-center gap-3 rounded-[var(--radius-md)] px-3 py-2.5 text-sm font-medium transition-colors",
                  pathname.startsWith("/super-admin/security") ? "bg-[var(--color-primary)]/10 text-[var(--color-primary)]" : "text-[var(--color-text-secondary)] hover:bg-[var(--color-bg)] hover:text-[var(--color-text)]",
                  collapsed && "lg:justify-center"
                )}>
                  <ShieldAlert className="h-[18px] w-[18px] shrink-0" />
                  {!collapsed && <span className="truncate">Security Monitoring</span>}
                </Link>
              </li>
            )}
            {isSuperAdmin && (
              <li className="group relative">
                <Link href="/super-admin/subscriptions" onClick={onCloseMobile} className={cn(
                  "flex items-center gap-3 rounded-[var(--radius-md)] px-3 py-2.5 text-sm font-medium transition-colors",
                  pathname.startsWith("/super-admin/subscriptions") ? "bg-[var(--color-primary)]/10 text-[var(--color-primary)]" : "text-[var(--color-text-secondary)] hover:bg-[var(--color-bg)] hover:text-[var(--color-text)]",
                  collapsed && "lg:justify-center"
                )}>
                  <CreditCard className="h-[18px] w-[18px] shrink-0" />
                  {!collapsed && <span className="truncate">Subscriptions & Billing</span>}
                </Link>
              </li>
            )}
